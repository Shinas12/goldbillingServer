...
export class idsutil {
  ...
  async getIDSClientInstance(parentSpanInst, clientInstance = null, ...others) {
    ...
    try {
      ...
      bh = await this.sd_bmswzI1gwHPmORK9(bh, parentSpanInst);
  ...
  async sd_bmswzI1gwHPmORK9(bh, parentSpanInst) {
    const spanInst = this.tracerService.createSpan(
      'sd_bmswzI1gwHPmORK9',
      parentSpanInst
    );
    try {
      ...
      //appendnew_next_sd_bmswzI1gwHPmORK9
      return bh;
    } catch (e) {
      return await this.errorHandler(
        bh,
  ...
  async sd_NDvR3YN7qjsaaO0E(bh, parentSpanInst) {
    ...
    try {
      ...
      if (
        ...
        bh = await this.sd_49FVavBtfTmIt1Rk(bh, parentSpanInst);
        otherwiseFlag = false;
      }
      this.tracerService.sendData(spanInst, bh);
  ...
  async sd_J9L3oEbqPtl8tfbi(bh, parentSpanInst) {
    ...
    try {
      ...
      this.tracerService.sendData(spanInst, bh);
      bh = await this.sd_eo1tJF6LQQIuW6kZ(bh, parentSpanInst);
  ...
  async sd_eo1tJF6LQQIuW6kZ(bh, parentSpanInst) {
    const spanInst = this.tracerService.createSpan(
      'sd_eo1tJF6LQQIuW6kZ',
      parentSpanInst
    );
    try {
      ...
      this.tracerService.sendData(spanInst, bh);
      //appendnew_next_sd_eo1tJF6LQQIuW6kZ
      return bh;
    } catch (e) {
      return await this.errorHandler(
        bh,
  ...
  async sd_49FVavBtfTmIt1Rk(bh, parentSpanInst) {
    const spanInst = this.tracerService.createSpan(
      'sd_49FVavBtfTmIt1Rk',
      parentSpanInst
    );
    try {
      ...
      this.tracerService.sendData(spanInst, bh);
      //appendnew_next_sd_49FVavBtfTmIt1Rk
      return bh;
    } catch (e) {
      return await this.errorHandler(
        bh,
  ...
  async errorHandler(bh, e, src, parentSpanInst?, functionName?) {
    ...
    if (
      false ||
      (await this.sd_icmc68pVkr66dHoJ(bh, parentSpanInst))
  ...
  async sd_icmc68pVkr66dHoJ(bh, parentSpanInst) {
    const nodes = ['handleTokenExpiry'];
    if (nodes.includes(bh.errorSource)) {
      bh = await this.sd_EgUC5B8p6Vp4y70s(bh, parentSpanInst);
      //appendnew_next_sd_icmc68pVkr66dHoJ
      return true;
    }
    return false;
  }
